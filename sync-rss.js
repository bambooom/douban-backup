...
(async () => {
  ...
  // notify.sendNotify(`豆瓣→Notion`,message.join('\n'));
  if (message == `没有需要同步的新内容,快到豆瓣上记录吧~`) {}else{notify.sendNotify(`豆瓣→Notion`,message.join('\n'));};
})();
...
async function handleFeed(feed, category) {
  ...
  if(feed.length!=0){ message.push(`同步至Notion ${feed.length}个项目:`);}
...
async function addToNotion(itemData, category) {
  ...
  message.push(`    ${itemData[DB_PROPERTIES.TITLE]}`);
  try {
